...
- Orca Buddy v0.1.6 retains saved Wi-Fi credentials and directly reconnects to
...
- Orca Buddy v0.1.6 pairing diagnosis now distinguishes no USB serial port from
...
- Orca Buddy v0.1.6 removes the failed combined-record coupling: Wi-Fi uses a
  separate small checksummed NVS record while pairing uses its own NVS/SPIFFS
  fallback record. Therefore Wi-Fi save/forget never rewrites or depends on
  pairing CA/bearer storage. USB CDC port selection now waits through a bounded
  ...
  failures. Current v0.1.6 artifact still needs Launcher installation, Wi-Fi
...
- Orca Buddy: install current v0.1.6 artifact through Launcher, then pair through
