...
- Orca Buddy v0.1.5 retains saved Wi-Fi credentials and directly reconnects to
...
- Orca Buddy v0.1.5 pairing diagnosis now distinguishes no USB serial port from
...
- Orca Buddy v0.1.5 removes the failed combined-record coupling: Wi-Fi uses a
  ...
  contract (3), serial sender/port selection (6), plugin (14), bridge (28),
  and Launcher build (1,074,933 bytes) pass. Current v0.1.5 artifact still
  needs Launcher installation, Wi-Fi save/reboot proof, USB pairing, then WSS
  bridge proof.
...
- Orca Buddy: install current v0.1.5 artifact through Launcher, then pair through
