...
3. Annotated `claude-buddy-v1.3.0` tag.
   - Records an historical source baseline and the observed raw binary SHA-256
     `e969dbfe9fd2f444c477e6df0d9bcd9c7d564ce203f80c2f4815d0e58bcb5051`.
   - It is not a Launcher release. Exact rebuild equality is not required:
     the historical source used floating dependencies and a clean rebuild now
     resolves newer compatible M5 library versions.
...
with its original `no_ota.csv` layout only to confirm that recovered source
still builds. Record its SHA-256 alongside the observed historical binary;
they need not match because historical dependencies were not pinned. Neither
image may be staged, installed, or flashed.
...
- If a clean v1.3.0 rebuild differs from recorded SHA-256, record dependency
  drift and both hashes in historical provenance; continue only after its
...
1. Clean rebuild v1.3.0 source baseline, record its SHA-256 and dependency
   drift beside the observed historical artifact SHA-256.
