...
class CardputerAdvKeyboardReader : public KeyboardReader {
public:
  void begin() override {
    m5::In_I2C.begin(I2C_NUM_1, GPIO_NUM_8, GPIO_NUM_9);
    ...
    _tca.matrix(7, 8);
    _tca.flush();
    pinMode(GPIO_NUM_11, INPUT_PULLUP);
    _tca.enableInterrupts();
  }

  void update() override {
    if (!_ok) return;

    // The official reader waits for the GPIO11 interrupt edge. Polling the
    // FIFO as well keeps input reliable if an edge was missed or the INT line
    // was already low before attachInterrupt ran.
    uint8_t count = _tca.available();
    uint8_t guard = 12;
    while (count-- && guard--) {
      uint8_t event = _tca.getEvent();
      if (event == 0) break;
      applyEvent(event);
    }

    if (guard != 12) {
      _tca.writeRegister8(TCA8418_REG_INT_STAT, 1);
    }
  }
...
private:
  ...
  static void remap(uint8_t rawRow, uint8_t rawCol, Point2D_t* out) {
    out->x = rawRow * 2 + (rawCol > 3 ? 1 : 0);
    out->y = (rawCol + 4) % 4;
  }

  void applyEvent(uint8_t event) {
    uint8_t key = event & 0x7F;
    if (key == 0) return;
    key--;

    Point2D_t point;
    remap(key / 10, key % 10, &point);
    if (point.x < 0 || point.x >= 14 || point.y < 0 || point.y >= 4) return;

    bool pressed = (event & 0x80) != 0;
    for (auto it = _key_list.begin(); it != _key_list.end(); ++it) {
      if (*it == point) {
        if (!pressed) _key_list.erase(it);
        return;
      }
    }
    if (pressed) _key_list.push_back(point);
  }
};
}  // namespace
...
void halInit() {
  ...
  // Skip auto keyboard init so we can use a Cardputer-Adv-specific reader
  // that explicitly binds the TCA8418 to the documented I2C pins and polls
  // the event FIFO directly.
  M5Cardputer.begin(cfg, false);
  M5Cardputer.Keyboard.begin(std::unique_ptr<KeyboardReader>(new CardputerAdvKeyboardReader()));
  M5.Speaker.begin();
