...
```bash
# From monorepo root: build, package, and stage raw app image for Launcher OTA.
./cardputer build claude-buddy
./cardputer stage claude-buddy
```

Direct serial flash, erase, and merged-image tools live under
`scripts/recovery/`. They are recovery-only and never replace Launcher OTA for
routine installs.
...
Normal install uses raw app image, not merged full-flash binary:

```bash
./cardputer stage claude-buddy
```

On device: Launcher → Settings → USB MSC; run staging; exit USB MSC; select the
new file in `tools/`; choose Install.
